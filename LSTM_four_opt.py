...
BATCH_SIZE    = 128
RNG_STATE     = 42
OVERSAMPLE_POS= 5  # Increased to help with sensitivity
START_DATE    = "2010-01-01"  # used if your CSVs lack explicit dates
MIN_SPECIFICITY = 0.98  # Minimum required specificity (constraint)
...
def build_lstm(seq_len=SEQ, class_weight=None):
...
def tune_threshold(y_val, p_val, min_specificity=0.98):  # Multi-objective with specificity constraint
    best_t, best_score = 0.5, -1
    for t in np.linspace(0.05, 0.95, 19):
        yhat = (p_val >= t).astype(int)
        sens, spec = sens_spec(y_val, yhat)
        
        # Calculate POD and timeliness for this threshold
        # For validation, we'll use a simplified approach
        # (In practice, you'd need the full validation sequences for accurate POD/timeliness)
        pod = sens  # Approximation: POD ≈ sensitivity for validation
        timeliness = sens  # Approximation: timeliness ≈ sensitivity for validation
        
        # Check specificity constraint
        if spec < min_specificity:
            continue  # Skip this threshold if specificity too low
            
        # Multi-objective score: equal weight for sens, POD, timeliness
        # (specificity is handled as constraint)
        score = (sens + pod + timeliness) / 3
        
        if score > best_score:
            best_score, best_t = score, float(t)
    
    # If no threshold meets specificity constraint, use the one with highest specificity
    if best_t == 0.5:  # No threshold was selected
        best_spec, best_t = 0, 0.5
        for t in np.linspace(0.05, 0.95, 19):
            yhat = (p_val >= t).astype(int)
            sens, spec = sens_spec(y_val, yhat)
            if spec > best_spec:
                best_spec, best_t = spec, float(t)
        print(f"Warning: No threshold met specificity >= {min_specificity}, using best spec={best_spec:.3f}")
...
for S in SIGNALS:
    ...
    # Calculate class weights to help with imbalanced data
    class_counts = np.bincount(Ytr)
    total_samples = len(Ytr)
    class_weight = {0: total_samples / (2 * class_counts[0]), 
                    1: total_samples / (2 * class_counts[1])} if len(class_counts) == 2 else None
    
    # Fit
    tf.keras.backend.clear_session()
    model = build_lstm(SEQ, class_weight)
    ...
    if len(Xval):
        model.fit(Xtr, Ytr, validation_data=(Xval, Yval), epochs=EPOCHS,
                  batch_size=BATCH_SIZE, verbose=0, callbacks=[es], class_weight=class_weight)
        Pval = model.predict(Xval, verbose=0).squeeze()
        t_star = tune_threshold(Yval, Pval, MIN_SPECIFICITY)
    else:
...
if summary_all and summary_bh:
    ...
    df_combined.to_csv("LSTM_results_four_opt_check.csv")
    print(f"Combined results saved to: LSTM_results_four_opt_check.csv")
